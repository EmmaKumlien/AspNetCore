...
public abstract class Repo<TEntity>(DataContext context) where TEntity : class
{
	...
	public virtual async Task<ResponsResult> UpdateAsync(Expression<Func<TEntity, bool>> predicate, TEntity updatedEntity)
	{
		try
		{
			var result = await _context.Set<TEntity>().FirstOrDefaultAsync(predicate);
	...
	public virtual async Task<ResponsResult> DeleteAsync(Expression<Func<TEntity, bool>> predicate)
	{
		try
		{
			var result = await _context.Set<TEntity>().FirstOrDefaultAsync(predicate);
